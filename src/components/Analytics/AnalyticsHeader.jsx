	...
	return (
			<>
				<div>
					...
					<p className="text-sm font-medium text-[var(--text-600)] mt-1">
						Real-time market analysis and insights
					</p>
				</div>

				<div className="max-md:hidden flex justify-start w-1/2 m-10 items-center gap-10">
					<p className="text-lg font-semibold text-[var(--text-950)] flex items-center gap-1">
						{latest?.toFixed(2)}
						{priceChange ? (
							<span
					...
					<div className="flex flex-col gap-2 text-sm font-medium ">
						<h1 className="font-semibold text-lg text-[var(--text-600)]">
							Exchange
						</h1>
						<p className="text-[var(--text-950)]">
							{exchange}
						</p>
					</div>
				</div>
